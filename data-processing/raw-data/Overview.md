...
**Strengths**:
- **Event-level granularity**: Individual incidents, not aggregated
- **Real-time updates**: Data updated weekly with new events
- **Source transparency**: All events linked to media/report sources
- **Comprehensive coverage**: Political violence + protests + strategic developments
- **Precise geolocation**: Latitude/longitude for mapping
- **Actor detail**: Named groups, not just generic categories
- **Context-rich**: Detailed notes field with event descriptions
- **Fatality estimates**: Reported deaths per event
- **Long time series**: 27+ years of consistent data

**Unique Capabilities**:
- Track protest movements over time and space
- Identify violence hotspots at sub-national level
- Analyze actor networks and alliances
- Monitor real-time conflict escalation
- Study riot patterns and state response
...
**For event-level visualization and analysis → Use ACLED**
- Most comprehensive event coverage
- Includes protests and non-violent events
- Real-time updates available
- Best for: mapping, time series, hotspot analysis, actor networks

**For academic conflict research → Use UCDP**
- Established standard in conflict studies
- Conservative coding (higher confidence)
- Longer historical coverage (1946 vs 1997)
- Best for: academic papers, cross-study comparisons, fatality estimates

**For religious dimensions → Use Religious Cleavages**
- Specialized religious coding
- Links religion to conflict dynamics

**For demographic context → Use Epac + WRP**
- Ethnic and religious composition by country
- Historical religious trends

### ACLED vs UCDP: Key Differences

| Feature | ACLED | UCDP |
|---------|-------|------|
| **Events** | 2.37M (1997-2024) | ~10K (1989-2024) |
| **Scope** | All political violence + protests | Only organized armed conflict |
| **Threshold** | No minimum fatalities | ≥25 deaths/year at conflict level |
| **Granularity** | Individual events | Events (GED) + annual summaries |
| **Updates** | Weekly (real-time) | Annual releases |
| **Coverage start** | 1997 | 1946 (conflict), 1989 (events) |
| **Best for** | Sub-national patterns, protests | Long-term trends, war/peace |

**Recommendation**: Use ACLED as primary for visualizations (larger, more detailed). Cross-reference with UCDP for validation and historical context.
...
### Primary Dataset (ACLED)

**Geographic Visualizations**:
- Conflict density heatmaps
- Event location mapping with fatality markers
- Animation of conflict spread over time
- Sub-national violence patterns

**Temporal Analysis**:
- Daily/weekly event counts and trends
- Protest wave identification
- Violence escalation tracking
- Seasonal patterns

**Actor Analysis**:
- Armed group activity patterns
- State vs non-state violence
- Civilian targeting analysis
- Actor interaction networks

**Event Type Analysis**:
- Protest vs violence ratios
- Battle types and outcomes
- Remote violence trends (drones, IEDs)
- Strategic development patterns

### Supplementary Analysis

**Historical Context** (UCDP):
- Long-term conflict trends (1946-2024)
- War onset and termination
- Conflict intensity classification
- Academic research validation

**Religious Dimensions** (Religious Cleavages):
- Religion's role in conflict
- Religious fractionalization effects
- Sectarian violence patterns

**Demographic Background** (Epac + WRP):
- Ethnic composition and conflict
- Religious demographics over time
- Group grievances and mobilization

---

## Data Processing Recommendations

### For ACLED (1.32 GB file)

**Memory Management**:
```python
# Load in chunks
for chunk in pd.read_csv('ACLED_2025-10-29.csv', chunksize=100000):
    process(chunk)

# Or filter while loading
df = pd.read_csv('ACLED_2025-10-29.csv',
                 usecols=['event_date', 'country', 'latitude', 'longitude', 'fatalities'],
                 parse_dates=['event_date'])
```

**Recommended Filters for Visualization**:
- **Geographic**: Filter to specific countries or regions
- **Temporal**: Select date ranges (e.g., 2020-2024)
- **Event type**: Focus on specific violence/protest types
- **Fatalities**: Filter events with fatalities > 0
- **Actors**: Select specific armed groups or actors

**Typical Reduction**:
- Raw: 2.37M rows → Filtered: 10K-100K rows (95-99% reduction)
- This brings data into manageable size for D3.js

### For Other Datasets

All other datasets are small enough (<500 MB) to load fully into memory for processing.

---

## Version Information

- **ACLED**: 2025-10-29 export (real-time data through October 2024)
- **UCDP**: Version 25.1 (2025 release)
- **Religious Cleavages**: No version specified
- **Epac**: 2021 edition
- **WRP**: Version 1.1

## Data Quality Summary

| Dataset | Completeness | Temporal Coverage | Geographic Coverage | Update Frequency | Confidence |
|---------|--------------|-------------------|---------------------|------------------|------------|
| ACLED | ★★★★★ (comprehensive) | 1997-2024 | Global | Weekly | High |
| UCDP | ★★★★☆ (selective) | 1946-2024 | Global | Annual | Very High |
| Religious Cleavages | ★★★☆☆ (subset) | 1946-present | Conflict countries | Static | High |
| Epac | ★★★☆☆ (ethnic groups) | Country-specific | Global | Static (2021) | High |
| WRP | ★★★★☆ (religious) | 1945-2010 | 165+ countries | Static (ends 2010) | High |
